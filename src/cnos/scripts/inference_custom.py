...
def run_inference(template_dir, rgb, num_max_dets, conf_threshold, stability_score_thresh):
    ...
    results = detections.save_to_file(0, 0, 0, save_path, "custom", return_results=True)
    detections = convert_npz_to_json(idx=0, list_npz_paths=[save_path+".npz"])
    save_json_bop23(save_path+".json", detections)
    vis_img = visualize(rgb, detections, save_path=f"{template_dir}/cnos_results/vis.png")
